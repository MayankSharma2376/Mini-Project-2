...
import { Calendar, Users, Clock, Check, X, Download, UserCheck, AlertCircle, CheckCircle2 } from 'lucide-react';
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
          <div className="bg-blue-50 rounded-lg p-4 text-center">
            ...
            <div className="text-sm text-blue-700">Total Volunteers</div>
          </div>
      ...
      <div className="p-6">
        {volunteers.length === 0 ? (
          <div className="text-center py-8">
            ...
            <p className="text-gray-600">No accepted volunteers for this event</p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-gray-200">
                  ...
                  <th className="text-left py-3 px-4 font-semibold text-gray-700">Arrival Time</th>
                ...
                {volunteers.map((volunteer) => (
                  <tr key={volunteer.id} className="border-b border-gray-100 hover:bg-gray-50">
                    <td className="py-3 px-4">
                      <div className="flex items-center">
                        <div className="w-8 h-8 bg-blue-100 text-blue-600 rounded-full flex items-center justify-center mr-3 font-semibold">
                          {volunteer.volunteerName.charAt(0).toUpperCase()}
                        </div>
                        <div>
                          <p className="font-medium text-gray-900">{volunteer.volunteerName}</p>
                          <p className="text-sm text-gray-500">Applied: {new Date(volunteer.appliedAt).toLocaleDateString()}</p>
                        </div>
                      </div>
                    </td>
                    <td className="py-3 px-4">
                      <p className="text-sm text-gray-900">{volunteer.email}</p>
                      <p className="text-sm text-gray-500">{volunteer.phone || 'No phone'}</p>
                    </td>
                    <td className="py-3 px-4">
                      <span className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${
                        ...
                        volunteer.attendanceStatus === 'late' ? 'bg-yellow-100 text-yellow-800' :
                    ...
                    <td className="py-3 px-4">
                      <p className="text-sm text-gray-900">{volunteer.arrivalTime || '-'}</p>
                      {volunteer.notes && (
                        <p className="text-xs text-gray-500 mt-1">{volunteer.notes}</p>
                      )}
                    </td>
                    <td className="py-3 px-4">
